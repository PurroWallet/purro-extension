...
export class PurroEVMProvider implements EthereumProvider {
  ...
  private async initializeSelectedAddress() {
    ...
    try {
      ...
      if (
        ...
        this.providerManager.activeAccount
      ) {
        ...
        if (
          ...
          if (!this.lastEmitTime || now - this.lastEmitTime > 100) {
            ...
            this.emit('connect', {
              accounts: accountsToEmit,
              activeAccount: newAddress,
            });
  ...
  private init() {
    ...
    this.providerManager.on('accountsChanged', (accounts: string[]) => {
      ...
      if (
        JSON.stringify(this.lastEmittedAccounts) === JSON.stringify(accounts)
      ) {
        return;
      }
  ...
  private async getConnectedAccountForSite(): Promise<any> {
    try {
      ...
      const connectedSitesResult = await this.sendMessage(
        'GET_CONNECTED_SITES',
      ...
      const currentSite = connectedSites.find(
        (site: any) => site.origin === window.location.origin
      );

      if (currentSite) {
        // Get wallet address for this account
        const walletResult = await this.sendMessage('GET_WALLET_BY_ID', {
          accountId: activeAccount.id,
        });
  ...
  private async handleGetAccounts(): Promise<string[]> {
    try {
      ...
      if (
        this.providerManager.isConnected &&
        this.providerManager.accounts.length > 0
      ) {
      ...
      if (result && result.address) {
        ...
        if (
          JSON.stringify(this.lastEmittedAccounts) !==
          JSON.stringify(accountsToReturn)
        ) {
          const now = Date.now();
          if (!this.lastEmitTime || now - this.lastEmitTime > 100) {
  ...
  private async handleSignTransaction(params: any): Promise<string> {
    ...
    if (typeof result === 'string') {
      ...
      throw this.createProviderError(
        4001,
        'No signed transaction returned from signing process'
      );
    }
  }
  ...
  private async handlePersonalSign(params: any): Promise<string> {
    ...
    if (typeof result === 'string') {
      ...
      throw this.createProviderError(
        4001,
        'No signature returned from signing process'
      );
    }
  }
  ...
  private async handleGetTransactionCount(params: any): Promise<string> {
    if (!params || !Array.isArray(params) || params.length === 0) {
      throw this.createProviderError(
        4001,
        'Invalid transaction count parameters'
      );
    }
  ...
  private async handleGetTransactionReceipt(params: any): Promise<any> {
    if (!params || !Array.isArray(params) || params.length === 0) {
      throw this.createProviderError(
        4001,
        'Invalid transaction receipt parameters'
      );
    }
