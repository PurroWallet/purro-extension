...
export const evmWalletKeyUtils = {
  deriveFromSeed: (seed: Buffer, accountIndex: number = 0): WalletKeys => {
    try {
      ...
      const ethereumWallet =
        ethers.HDNodeWallet.fromSeed(seed).derivePath(ethereumPath);
      ...
      throw new Error(
        `Failed to derive Ethereum wallet from seed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  },
  ...
  deriveFromMnemonic: (
    ...
    try {
      ...
      throw new Error(
        `Failed to derive Ethereum wallet from mnemonic: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  },
  ...
  fromPrivateKey: (privateKey: string): WalletKeys => {
    try {
      ...
      throw new Error(
        `Failed to create Ethereum wallet from private key: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  },
  ...
  isValidPrivateKey: (privateKey: string): boolean => {
    try {
      ...
      const normalizedInput = privateKey.startsWith('0x')
        ? privateKey
        : `0x${privateKey}`;
...
export const suiWalletKeyUtils = {
  deriveFromSeed: (seed: Buffer, accountIndex: number = 0): WalletKeys => {
    try {
      ...
      throw new Error(
        `Failed to derive Sui wallet from seed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  },
  ...
  deriveFromMnemonic: (
    ...
    try {
      ...
      throw new Error(
        `Failed to derive Sui wallet from mnemonic: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  },
  ...
  fromPrivateKey: (privateKey: string): WalletKeys => {
    try {
      ...
      throw new Error(
        `Failed to create Sui wallet from private key: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  },
...
export const solanaWalletKeyUtils = {
  deriveFromSeed: (seed: Buffer, accountIndex: number = 0): WalletKeys => {
    try {
      ...
      throw new Error(
        `Failed to derive Solana wallet from seed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  },
  ...
  deriveFromMnemonic: (
    ...
    try {
      ...
      throw new Error(
        `Failed to derive Solana wallet from mnemonic: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  },
    ...
    try {
      ...
      if (privateKey.startsWith('[') && privateKey.endsWith(']')) {
        ...
        try {
          ...
          throw new Error(
            'Invalid private key format - not hex, array, or Base58'
          );
        }
      }
      ...
      throw new Error(
        `Failed to create Solana wallet from private key: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  },
  ...
  fromSecretKeyArray: (secretKeyArray: number[]): WalletKeys => {
    try {
      ...
      throw new Error(
        `Failed to create Solana wallet from secret key array: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  },
  ...
  convertPrivateKeyFormat: (
    ...
    try {
      ...
      throw new Error(
        `Failed to convert private key format: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  },
};
...
export const multiChainWalletUtils = {
  deriveAllFromMnemonic: (
    ...
    try {
      ...
      throw new Error(
        `Failed to derive multi-chain wallets: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  },
  ...
  deriveAllFromSeed: (
    ...
    try {
      ...
      throw new Error(
        `Failed to derive multi-chain wallets from seed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  },

  deriveAllFromPrivateKey: (
    privateKey: string,
    chain: ChainType
  ): WalletKeys => {
    try {
      ...
      throw new Error(
        `Failed to derive multi-chain wallets from private key: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  },
  ...
  validatePrivateKeyForAllChains: (
    privateKey: string
  ): {
    ethereum: boolean;
    solana: boolean;
  ...
  detectChainFromPrivateKey: (privateKey: string): ChainType | null => {
    ...
    const validation =
      multiChainWalletUtils.validatePrivateKeyForAllChains(privateKey);
  ...
  getPrivateKeyInfo: (
    ...
    const validation =
      multiChainWalletUtils.validatePrivateKeyForAllChains(privateKey);
    const detectedChain =
      multiChainWalletUtils.detectChainFromPrivateKey(privateKey);
