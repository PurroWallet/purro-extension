...
export const accountHandler = {
  ...
  async getPrivateKeyIdByPrivateKey(
    privateKey: string
  ): Promise<string | null> {
    try {
      // Validate input
  ...
  async importSeedPhrase(data: ImportSeedPhraseData): Promise<string> {
    ...
    const authResult = await authValidator.validateAndGetPassword(
      data.password
    );
  ...
  async createAccountBySeedPhrase(data: {
    name?: string;
    icon?: string;
    seedPhraseId: string;
    password?: string;
    ...
    try {
      ...
      const mnemonic = await encryption.decrypt(
        seedPhraseData.data,
        authResult.password
      );
      const { ethereum, solana, sui } =
        multiChainWalletUtils.deriveAllFromMnemonic(mnemonic, derivationIndex);
      ...
      await Promise.all([
        ...
        storageHandler.updateSeedPhrase(
          data.seedPhraseId,
          { ...seedPhraseData, currentDerivationIndex: derivationIndex },
          [accountId]
        ),
  ...
  async importAccountByPrivateKey(data: {
    name: string;
    chain: ChainType;
    privateKey: string;
    password?: string;
    ...
    try {
      if (data.chain === 'eip155') {
        isValidPrivateKey = evmWalletKeyUtils.isValidPrivateKey(
          data.privateKey
        );
        ...
        isValidPrivateKey = solanaWalletKeyUtils.isValidPrivateKey(
          data.privateKey
        );
        if (isValidPrivateKey) {
        ...
        isValidPrivateKey = suiWalletKeyUtils.isValidPrivateKey(
          data.privateKey
        );
        if (isValidPrivateKey) {
    ...
    const isAlreadyImported = await this.isPrivateKeyAlreadyImported(
      data.privateKey
    );
    if (isAlreadyImported) {
    ...
    const newAccount: AccountInformation = {
      id: accountId,
      name:
        data.name || `Account ${accounts.length > 0 ? accounts.length + 1 : 1}`,
      icon: '🐱',
      source: 'privateKey',
    ...
    const encryptedPrivateKey = await encryption.encrypt(
      data.privateKey,
      authResult.password
    );
  ...
  async changePassword(data: {
    ...
    const isValidOldPassword = await encryption.verifyPassword(
      data.oldPassword,
      passwordStored.data,
      passwordStored.salt
    );
    ...
    const seedPhrasePromises = allSeedPhrases.map(
      async ({ id, data: seedPhraseData }) => {
        // Decrypt with old password
        const decryptedSeedPhrase = await encryption.decrypt(
          seedPhraseData.data,
          data.oldPassword
        );
        // Re-encrypt with new password
        const reEncryptedSeedPhrase = await encryption.encrypt(
          decryptedSeedPhrase,
          data.newPassword
        );
    ...
    const privateKeyPromises = allPrivateKeys.map(
      async ({ id, data: privateKeyData }) => {
        // Decrypt with old password
        const decryptedPrivateKey = await encryption.decrypt(
          privateKeyData,
          data.oldPassword
        );
        // Re-encrypt with new password
        const reEncryptedPrivateKey = await encryption.encrypt(
          decryptedPrivateKey,
          data.newPassword
        );
    ...
    const [seedPhraseUpdates, privateKeyUpdates, newPasswordEncrypted] =
      await Promise.all([
    ...
    try {
      const updatePromises = [
        ...
        chrome.storage.local.set({
          [STORAGE_KEYS.ACCOUNT_PASSWORD]: newPasswordEncrypted,
        }),
  ...
  async removeAccount(accountId: string): Promise<void> {
    ...
    try {
      ...
      if (account.source === 'privateKey' && account.privateKeyId) {
        removePromises.push(
          storageHandler.removePrivateKey(account.privateKeyId)
        );
      }
      ...
      if (account.source === 'seedPhrase' && account.seedPhraseId) {
        await storageHandler.removeAccountFromSeedPhrase(
  ...
  async removeSeedPhrase(data: {
    ...
    try {
      ...
      if (seedPhraseData.accountIds && seedPhraseData.accountIds.length > 0) {
        ...
        accountsToRemove = accountResults
          .filter(
            ({ account }) =>
              account &&
              account.source === 'seedPhrase' &&
              account.seedPhraseId === seedPhraseId
          )
      ...
      throw new Error(
        'Failed to remove seed phrase and associated accounts. Please try again.'
      );
    }
  },
  ...
  async removePrivateKey(privateKeyId: string): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        'Failed to remove private key and associated accounts. Please try again.'
      );
    }
  },
  ...
  async resetWallet(): Promise<void> {
    try {
      ...
      await Promise.all([authHandler.lock(), storageHandler.resetWallet()]);
    } catch (error) {
  ...
  async getPrivateKeyByAccountId(accountId: string): Promise<string> {
    ...
    if (account.source === 'privateKey' && account.privateKeyId) {
      ...
      const encryptedPrivateKey = await storageHandler.getPrivateKeyById(
        account.privateKeyId
      );
      if (!encryptedPrivateKey) {
        throw new Error('Private key not found');
      }
      privateKey = await encryption.decrypt(
        encryptedPrivateKey,
        session.password
      );
    } else if (
      ...
      const seedPhraseData = await storageHandler.getSeedPhraseById(
        account.seedPhraseId
      );
      if (!seedPhraseData) {
        throw new Error('Seed phrase not found');
      ...
      const evmWallet = evmWalletKeyUtils.deriveFromMnemonic(
        mnemonic,
  ...
  async exportPrivateKey(
    accountId: string,
    chainOrPassword: ChainType | string,
    ...
    if (account.source === 'privateKey' && account.privateKeyId) {
      ...
      if (!multiChainWalletUtils.isValidPrivateKey(privateKey, selectedChain)) {
        throw new Error(
          `The stored private key is not valid for chain ${selectedChain}`
        );
      }
    }
    // Case 2: Account derived from a seed phrase – derive key according to chain
      ...
      if (selectedChain === 'eip155') {
        privateKey = evmWalletKeyUtils.deriveFromMnemonic(
          mnemonic,
          account.derivationIndex
        ).privateKey;
      } else if (selectedChain === 'solana') {
        privateKey = solanaWalletKeyUtils.deriveFromMnemonic(
          mnemonic,
          account.derivationIndex
        ).privateKey;
      } else if (selectedChain === 'sui') {
        privateKey = suiWalletKeyUtils.deriveFromMnemonic(
          mnemonic,
          account.derivationIndex
        ).privateKey;
      } else {
  ...
  async importWatchOnlyWallet(
    ...
    const wallet: AccountWallet = {
      eip155:
        chain === 'eip155'
          ? { address: normalizedAddress, pathType: 'watchOnly' }
          : null,
      solana:
        chain === 'solana'
          ? { address: normalizedAddress, pathType: 'watchOnly' }
