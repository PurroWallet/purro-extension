import { runTest, TEST_MNEMONIC, TEST_PASSWORD, TEST_PRIVATE_KEY } from './test-utils';
...
export const runAccountHandlerTests = async () => {
    ...
    await runTest('Private key format validation', async () => {
        ...
        const validPrivateKeyWithout0x = validPrivateKey.startsWith('0x') ? validPrivateKey.slice(2) : validPrivateKey;
        ...
        const isValid66 = validPrivateKey && validPrivateKey.length === 66 && validPrivateKey.startsWith('0x');
        const isValid64 = validPrivateKeyWithout0x && validPrivateKeyWithout0x.length === 64 && /^[0-9a-fA-F]{64}$/.test(validPrivateKeyWithout0x);

        if (!isValid66 && !isValid64) {
            throw new Error('Test private key should be valid format (either 64 hex chars or 66 chars with 0x prefix)');
        }

        // Invalid private key checks
        for (const invalidKey of invalidPrivateKeys) {
            const cleanKey = invalidKey.startsWith('0x') ? invalidKey.slice(2) : invalidKey;
            const isValidFormat = cleanKey.length === 64 && /^[0-9a-fA-F]{64}$/.test(cleanKey);

            if (isValidFormat) {
                throw new Error(`Invalid private key should fail validation: ${invalidKey}`);
            }
        }
    ...
    await runTest('Wallet data structure validation', async () => {
        const wallet = {
            eip155: {
                ...
                publicKey: '0x04abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef',
        ...
        if (wallet.eip155.pathType !== 'bip44' && wallet.eip155.pathType !== 'imported') {
    ...
    await runTest('Data encryption structure validation', async () => {
        ...
        if (typeof encryptionData.iterations !== 'number' || encryptionData.iterations <= 0) {
    ...
    await runTest('Password strength requirements', async () => {
        ...
        for (const weakPassword of weakPasswords) {
            if (weakPassword.length >= 8 && /[A-Z]/.test(weakPassword) && /[0-9]/.test(weakPassword)) {
                throw new Error(`Weak password should not pass basic strength check: ${weakPassword}`);
            }
        }
