...
export const runSecurityTests = async () => {
    ...
    await runTest('Data integrity tampering detection', async () => {
        ...
        try {
            ...
            if (!(error as Error).message.includes('Data integrity verification failed')) {
    ...
    await runTest('Auto-lock scheduling and execution', async () => {
        ...
        if (chrome.alarms) {
            ...
            chrome.alarms.create = ((name: string, alarmInfo: chrome.alarms.AlarmCreateInfo) => {
        ...
        try {
            ...
            if (chrome.alarms) {
                ...
                if (timeDiff > 5000) { // Allow 5 second tolerance
                    throw new Error('Auto-lock alarm time should be approximately 30 minutes from now');
                }
            }
    ...
    await runTest('Password verification timing attack resistance', async () => {
        ...
        for (let i = 0; i < attempts; i++) {
            ...
            await encryption.verifyPassword(correctPassword, hashedPassword.data, hashedPassword.salt);
        ...
        for (let i = 0; i < attempts; i++) {
            const start = performance.now();
            await encryption.verifyPassword(wrongPassword1, hashedPassword.data, hashedPassword.salt);
            const end = performance.now();
            timings.push(end - start);
        }
        ...
        for (let i = 0; i < attempts; i++) {
            const start = performance.now();
            await encryption.verifyPassword(wrongPassword2, hashedPassword.data, hashedPassword.salt);
            const end = performance.now();
            timings.push(end - start);
        }
        ...
        console.log(`Timing stats: avg=${avgTiming.toFixed(2)}ms, variance=${variance.toFixed(2)}ms`);
        ...
        if (variance > avgTiming * 2) {
            console.warn('⚠️ High timing variance detected - potential timing attack vulnerability');
        }
