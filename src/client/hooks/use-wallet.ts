...
const useWallet = () => {
  ...
  const createWallet = useCallback(
    async ({ password, mnemonic }: CreateWalletData) => {
      try {
        setLoading(true);
        setError(null);
        const account = await sendMessage('CREATE_WALLET', {
          password,
          mnemonic,
        });
        return account;
  ...
  const createAccountFromSeedPhrase = useCallback(
    async ({
      name,
      icon,
      seedPhraseId,
      ...
      try {
        setLoading(true);
        setError(null);
        const account = await sendMessage('CREATE_ACCOUNT_FROM_SEED_PHRASE', {
          name,
          icon,
          seedPhraseId,
        });
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : 'Failed to create account from seed phrase';
        setError(errorMessage);
        throw new Error(errorMessage);
      } finally {
        setLoading(false);
  ...
  const importPrivateKey = useCallback(
    async ({
      ...
      try {
        setLoading(true);
        setError(null);
        const account = await sendMessage('IMPORT_PRIVATE_KEY', {
          privateKey,
          password,
          accountName,
          chain,
  ...
  const importSeedPhrase = useCallback(
    async ({
      ...
      try {
        setLoading(true);
        setError(null);
        const account = await sendMessage('IMPORT_SEED_PHRASE', {
          mnemonic,
          password,
          accountName,
        });
  ...
  const changeAccountName = useCallback(
    async (accountId: string, name: string) => {
      const changed = await sendMessage('CHANGE_ACCOUNT_NAME', {
        accountId,
        name,
      });
      return changed;
  ...
  const changeAccountIcon = useCallback(
    async (accountId: string, icon: string) => {
      const changed = await sendMessage('CHANGE_ACCOUNT_ICON', {
        accountId,
        icon,
      });
      return changed;
  ...
  const exportSeedPhrase = useCallback(
    async (seedPhraseId: string, password: string) => {
      const exported = await sendMessage('EXPORT_SEED_PHRASE', {
        seedPhraseId,
        password,
      });
      return exported;
  ...
  const exportPrivateKey = useCallback(
    async (privateKeyId: string, chain: ChainType, password: string) => {
      const exported = await sendMessage('EXPORT_PRIVATE_KEY', {
        privateKeyId,
        chain,
        password,
      });
  ...
  const importWatchOnlyWallet = useCallback(
    async (address: string, chain: ChainType, accountName: string) => {
      const imported = await sendMessage('IMPORT_WATCH_ONLY_WALLET', {
        address,
        chain,
        accountName,
      });
  ...
  const checkWatchOnlyAddressExists = useCallback(async (address: string) => {
    const exists = await sendMessage('IS_WATCH_ONLY_ADDRESS_EXISTS', {
      address,
    });
    return exists;
  }, []);

  const getAllSeedPhrases = useCallback(async (): Promise<
    SeedPhraseWithId[]
  > => {
    const seedPhrases: { id: string; data: SeedPhraseData }[] =
      await sendMessage('GET_ALL_SEED_PHRASES');
    return seedPhrases.map(sp => ({ id: sp.id, ...sp.data }));
  }, []);

  const changePassword = useCallback(
    async (oldPassword: string, newPassword: string) => {
      const changed = await sendMessage('CHANGE_PASSWORD', {
        oldPassword,
        newPassword,
      });
      return changed;
  ...
  const removeSeedPhrase = useCallback(
    async (seedPhraseId: string, password: string) => {
      const removed = await sendMessage('REMOVE_SEED_PHRASE', {
        seedPhraseId,
        password,
      });
      return removed;
