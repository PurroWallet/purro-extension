...
export const hyperEvmTokenKeys = {
  ...
  balances: (address: string) =>
    [...hyperEvmTokenKeys.all, 'balances', address] as const,
  prices: (addresses: string[]) =>
    [...hyperEvmTokenKeys.all, 'prices', addresses] as const,
  tokenData: (address: string) =>
    [...hyperEvmTokenKeys.all, 'tokenData', address] as const,
};
...
export const useHyperEvmTokenBalances = (address?: string) => {
  return useQuery({
    queryKey: address
      ? hyperEvmTokenKeys.balances(address)
      : ['hyperEvmTokens', 'disabled'],
    queryFn: async () => {
...
export const useHyperEvmTokenPrices = (tokenAddresses: string[]) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetchTokensInfoByAddresses(
        'hyperevm',
        tokenAddresses
      );
...
export const useHyperEvmTokens = () => {
  ...
  const processedPrices = useMemo(() => {
    ...
    if (response?.data && Array.isArray(response.data)) {
      const pricesMap: {
        [key: string]: { price: number; priceChange24h: number };
      } = {};
      response.data.forEach((tokenData: any) => {
        const address = normalizeAddress(tokenData.attributes.address);
        pricesMap[address] = {
  ...
  const processedTokens = useMemo((): HyperEvmToken[] => {
    ...
    return response.data.tokens
      .map(token => {
        ...
        const priceData = processedPrices[tokenAddress] ||
          tokenPrices[tokenAddress] || { price: 0, priceChange24h: 0 };
        const balance =
  ...
  const refetchTokenData = () => {
    return Promise.all([balancesQuery.refetch(), pricesQuery.refetch()]);
  };

  // Find token by address
  const findTokenByAddress = (address: string): HyperEvmToken | undefined => {
    return processedTokens.find(
  ...
  const getTokenPrice = (address: string): number => {
    ...
    return (
      processedPrices[tokenAddress]?.price ||
  ...
  const getTokenPriceChange = (address: string): number => {
    const tokenAddress = normalizeAddress(address);
    return (
      processedPrices[tokenAddress]?.priceChange24h ||
      tokenPrices[tokenAddress]?.priceChange24h ||
      0
    );
  };
