...
import {
  fetchArbitrumTokensOptimized,
  fetchBaseTokensOptimized,
  fetchEthereumTokensOptimized,
...
export const useAlchemyTokens = () => {
  ...
  const tokenAddressesByChain = useMemo(() => {
    ...
    allEvmTokensQuery.data.forEach(chainData => {
      if (chainData) {
        const chainAddresses = chainData.tokens.map(
  ...
  const priceQueriesConfig = useMemo(() => {
    ...
    if (isEthereumActive) {
      queries.push({
        queryKey: [
          QueryKeys.ETHEREUM_TOKENS,
          'prices',
          tokenAddressesByChain.ethereum,
        ],
        queryFn: () =>
          tokenAddressesByChain.ethereum.length > 0
    ...
    if (isBaseActive) {
      queries.push({
        ...
        queryFn: () =>
          tokenAddressesByChain.base.length > 0
    ...
    if (isArbitrumActive) {
      queries.push({
        queryKey: [
          QueryKeys.ARBITRUM_TOKENS,
          'prices',
          tokenAddressesByChain.arbitrum,
        ],
        queryFn: () =>
          tokenAddressesByChain.arbitrum.length > 0
      ...
      if (chainData && chainData.tokens) {
        ...
        chainData.tokens.forEach(token => {
          try {
            ...
            const usdPrice = chainPrices[tokenAddress]
              ? parseFloat(chainPrices[tokenAddress])
              : undefined;
  ...
  const isLoading =
    allEvmTokensQuery.isLoading || priceQueries.some(query => query.isLoading);

  const hasError =
    !!allEvmTokensQuery.error || priceQueries.some(query => !!query.error);
  ...
  return {
    ...
    // Error states
    error:
      allEvmTokensQuery.error || priceQueries.find(query => query.error)?.error,
    hasError,
