...
import useTestnetTokensStore, { TestnetToken } from "./use-testnet-tokens-store";
import { UnifiedToken } from "../components/token-list";
import { getTestnetNativeBalance, getTestnetTokenBalance } from "../utils/testnet-rpc";
    ...
    try {
        ...
        const walletAddress = useMemo(() => {
            try {
                ...
                if (address) {
                    console.log('🔍 Testnet wallet address:', address.substring(0, 8) + '...');
                }
        ...
        useEffect(() => {
            try {
                ...
                const updateBalances = async () => {
                    try {
                        console.log('🚀 Updating testnet balances for', tokensCount, 'tokens...');
                        setIsLoading(true);
                        setHasError(false);
                            ...
                            tokens.map(async (token) => {
                                try {
                                    if (token.isNative) {
                                        ...
                                        const { balance, balanceFormatted } = await getTestnetNativeBalance(walletAddress);
                                        console.log(`✅ ${token.symbol}:`, balanceFormatted, 'HYPE');
                                        updateTokenBalance(token.address, balance, balanceFormatted, 0, 0);
                                    } else {
                                        // Get ERC-20 token balance
                                        const { balance, balanceFormatted } = await getTestnetTokenBalance(
                                            token.address,
                                            walletAddress,
                                            token.decimals
                                        );
                                    ...
                                    console.error(`❌ Failed to get ${token.symbol} balance:`, error instanceof Error ? error.message : String(error));
                                }
                            })
                        );
