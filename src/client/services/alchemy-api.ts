...
class AlchemyCircuitBreaker {
    ...
    static shouldAllowRequest(): boolean {
        ...
        if (this.failureCount >= this.FAILURE_THRESHOLD) {
            const timeRemaining = Math.ceil((this.RESET_TIMEOUT - (now - this.lastFailureTime)) / 1000);
            console.warn(`🚫 Circuit breaker: API calls blocked due to ${this.failureCount} consecutive failures (${timeRemaining}s remaining)`);
            return false;
        }
        ...
        if (this.serviceUnavailableCount >= this.SERVICE_UNAVAILABLE_THRESHOLD) {
            const timeRemaining = Math.ceil((this.SERVICE_UNAVAILABLE_RESET_TIMEOUT - (now - this.lastServiceUnavailableTime)) / 1000);
            if (timeRemaining > 0) {
                console.warn(`🚫 Circuit breaker: API calls blocked due to service unavailable (${timeRemaining}s remaining)`);
                return false;
            }
        }
    ...
    static recordFailure(isServiceUnavailable: boolean = false): void {
        ...
        if (isServiceUnavailable) {
            ...
            console.warn(`⚠️ Circuit breaker: Service unavailable ${this.serviceUnavailableCount}/${this.SERVICE_UNAVAILABLE_THRESHOLD}, total failures ${this.failureCount}/${this.FAILURE_THRESHOLD}`);
        } else {
            console.warn(`⚠️ Circuit breaker: Recorded failure ${this.failureCount}/${this.FAILURE_THRESHOLD}`);
        }
    }

    static recordSuccess(): void {
        if (this.failureCount > 0 || this.serviceUnavailableCount > 0) {
            console.log(`✅ Circuit breaker: Reset after success (was ${this.failureCount} failures, ${this.serviceUnavailableCount} service unavailable)`);
            this.failureCount = 0;
            this.serviceUnavailableCount = 0;
        }
    }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 503) {
                    console.warn(`⚠️ Alchemy API overloaded (503) - attempt ${attempt}/${retries}`);
                    if (attempt < retries) {
                ...
                if (response.status >= 500) {
                    console.warn(`⚠️ Server error (${response.status}) - attempt ${attempt}/${retries}`);
                    if (attempt < retries) {
                ...
                throw new Error(`Failed to fetch token balances: HTTP ${response.status} ${response.statusText}`);
            }
            ...
            console.log(`✅ Successfully fetched token balances on attempt ${attempt}`);
            return data.result;
...
const getTokensMetadata = async (
    ...
    console.log(`🔍 Getting metadata for ${contractAddresses.length} tokens on chain ${chainId}`);
    ...
    if (missingAddresses.length > 0) {
        console.log(`🚀 Fast batch fetching ${missingAddresses.length} token metadata...`);
        ...
        try {
            ...
            const successCount = Object.values(newMetadata).filter(m => m.symbol !== 'UNKNOWN').length;
            const fallbackCount = Object.values(newMetadata).filter(m => m.symbol === 'UNKNOWN').length;

            if (fallbackCount > 0) {
                console.warn(`⚠️ ${fallbackCount}/${missingAddresses.length} tokens used fallback metadata (API errors/timeouts)`);
            }
            console.log(`✅ Successfully fetched ${successCount}/${missingAddresses.length} token metadata`);

        } catch (error) {
            console.error('❌ Batch fetch failed or timed out, using fallback for all:', error);
        ...
        if (Object.keys(newMetadata).length > 0) {
            ...
            console.log(`💾 Cached ${Object.keys(newMetadata).length} metadata entries`);
        }
    }
...
export const fetchEthereumTokensOptimized = async (address: string): Promise<ChainTokenData> => {
    try {
        ...
        const balances = await fetchAlchemyTokenBalances(ENDPOINTS.ALCHEMY_ETH_MAINNET, address);
...
export const fetchBaseTokensOptimized = async (address: string): Promise<ChainTokenData> => {
    try {
        ...
        const balances = await fetchAlchemyTokenBalances(ENDPOINTS.ALCHEMY_BASE_MAINNET, address);
        AlchemyCircuitBreaker.recordSuccess();
...
export const fetchArbitrumTokensOptimized = async (address: string): Promise<ChainTokenData> => {
    try {
        ...
        const balances = await fetchAlchemyTokenBalances(ENDPOINTS.ALCHEMY_ARB_MAINNET, address);
        AlchemyCircuitBreaker.recordSuccess();
...
export const fetchAllEvmTokensOptimized = async (address: string): Promise<ChainTokenData[]> => {
    try {
        ...
        const endTime = Date.now();
        const totalTokens = ethereumTokens.tokens.length + baseTokens.tokens.length + arbitrumTokens.tokens.length;

        console.log(`✅ Fetched ${totalTokens} tokens across all chains in ${endTime - startTime}ms`);
...
export const clearChainTokenCache = async (chain: 'ethereum' | 'base' | 'arbitrum') => {
