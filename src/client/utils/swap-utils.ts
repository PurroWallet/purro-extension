 ...
 */
export const formatMaxBalance = (balance: number, decimals: number = 18): string => {
    ...
    return balance.toFixed(Math.min(precision, maxDisplayDecimals)).replace(/\.?0+$/, '');
};
...
export const estimateGasCost = (
    ...
    if (!token) {
        return {
            ...
            gasCostEth: (GAS_ESTIMATION_CONSTANTS.NATIVE_TRANSFER_GAS * GAS_ESTIMATION_CONSTANTS.DEFAULT_GAS_PRICE) / 1e18,
            gasCostUsd: 0,
...
export const hasEnoughBalanceWithGas = (
    ...
    const gasCost = gasEstimate?.gasCostEth || GAS_ESTIMATION_CONSTANTS.NATIVE_GAS_RESERVE;
