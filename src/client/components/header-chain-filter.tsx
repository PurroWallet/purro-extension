...
interface HeaderChainFilterProps {
  isOpen: boolean;
  onToggle: () => void;
  selectedFilter: ChainFilter;
  onFilterChange: (filter: ChainFilter) => void;
  className?: string;
}

const HeaderChainFilter = ({
  isOpen,
  onToggle,
  selectedFilter,
  onFilterChange,
  className,
}: HeaderChainFilterProps) => {
  ...
  // Build an array of active network slugs
  ...
  const handleFilterSelect = (filter: ChainFilter) => {
  ...
  // Get the selected chain logo for display
  const getSelectedChainDisplay = () => {
    if (selectedFilter === 'all') {
      // Show all chain logos stacked like in supported-chains-dropdown
      return (
    ...
    // Show single chain logo for specific filter
    const selectedChain = sortedChains.find(chain => {
      const slug = CHAIN_ID_TO_SLUG[chain.chainId];
      return slug === selectedFilter;
    });
    return (
      <img
        src={selectedChain?.logo || hyperliquidLogo}
        alt="Selected Chain"
        className="size-5 rounded-full"
      />
    );
          ...
          <AnimatePresence>
            ...
            >
              ...
              <button
                ...
                className="w-full flex items-center justify-between p-3 hover:bg-white/5 transition-colors first:rounded-t-lg whitespace-nowrap cursor-pointer"
              >
              ...
              {sortedChains.map(chain => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-3 hover:bg-white/5 transition-colors last:rounded-b-lg whitespace-nowrap cursor-pointer"
                  >
                    <div className="flex items-center space-x-3">
                      <img
                        src={chain.logo}
...
export default HeaderChainFilter;
