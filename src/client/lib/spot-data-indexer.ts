...
export default class SpotDataIndexer {
    ...
    constructor(spotData: HyperliquidApiSpotAssetContext) {
        ...
        if (!spotData || !Array.isArray(spotData) || spotData.length < 2) {
            throw new Error('Invalid spot data format. Expected array with 2 elements.');
        }
    ...
    static isValidSpotData(data: unknown): data is HyperliquidApiSpotAssetContext {
        return (
            data !== null &&
            typeof data === 'object' &&
            Array.isArray(data) &&
    ...
    private getCoinIdentifier(coin: string, tokenInfo?: HyperliquidToken): string | undefined {
        ...
        for (const [universeName, item] of this.universeByName) {
            if (universeName.includes(upperCoin) || universeName.includes(`${upperCoin}/`)) {
                return `@${item.index}`;
            }
        }
    ...
    getSpotInfo(symbols: string[]): SpotInfoResult {
        ...
        symbols.forEach(symbol => {
            ...
            const priceData =
                universeIndex >= 0 && this.priceData[universeIndex] ? this.priceData[universeIndex] : null;
    ...
    processUserBalances(balanceResponse: BalanceResponse): UserBalance[] {
        ...
        return balanceResponse.balances.map(balance => {
            ...
            const totalAmount = parseFloat(balance.total);
            const marketValue = currentPrice && totalAmount ? currentPrice * totalAmount : undefined;
    ...
    getBalanceForCoin(balances: UserBalance[], coin: string): UserBalance | undefined {
    ...
    getUniverseByTokenIndex(tokenIndex: number): { name: string; identifier: string } | undefined {
        // Special case for USDC
        ...
        for (const [universeName, item] of this.universeByName) {
            if (item.tokens.includes(tokenIndex)) {
                return {
                    name: universeName,
                    identifier: universeName.toUpperCase() === 'PURR/USDC' ? 'PURR/USDC' : `@${item.index}`,
                };
            }
        }
        return undefined;
